...
const ProjectTile: NextPage<Props> = ({
  id, title, content, sourceLink, previewLink,
  createdDate, completedDate, tags
}: Props) => {
  ...
  return (
    <div className="card" key={id}>
      ...
      {(createdDate || completedDate) && <div className="card__date">
        {createdDate &&
          <p>Created Date: <span className="italic">{createdDate?.toString()}</span></p>
        }

        {completedDate &&
          <p>Completed Date: <span className="italic">{completedDate?.toString()}</span></p>
        }
      </div>}
