...
const CustomDirective = (content) => {
  ...
  if (result.includes(blockKey)) result = atBlock(result);
  if (result.includes(linkKey)) result = atLink(result);
  if (result.includes(markerKey)) result = atMarker(result);
  if (result.includes(pgKey)) result = atPg(result);
